...
### **D. Unit Architecture (Inheritance Strategy)**
To ensure code reusability and consistent networking, all units **MUST** follow this hierarchy:

1.  **`BaseUnit` (abstract partial class : CharacterBody2D)**
    * **Purpose:** Handles logic shared by ALL entities.
    * **Responsibilities:**
        * `[Export] CurrentHealth` & `MaxHealth` (Synced via MultiplayerSynchronizer).
        * `TakeDamage(int amount)`: Server-side logic to reduce HP.
        * `Die()`: Handles `QueueFree()` and death VFX RPCs.
        * `UnitType`: Abstract property.

2.  **`DefenderUnit` (abstract partial class : BaseUnit)**
    * **Purpose:** Static units placed on the grid.
    * **Responsibilities:**
        * **No Movement logic.**
        * `ActionTimer`: A server-side timer that triggers the unit's ability (Shoot/Produce Milk) every X seconds.

3.  **`AttackerUnit` (abstract partial class : BaseUnit)**
    * **Purpose:** Mobile units spawning from the right.
    * **Responsibilities:**
        * **Movement Logic:** Moves `Vector2.Left * Speed` in `_PhysicsProcess` (Server only).
        * **Combat:** Detects collision with `DefenderUnit`, stops moving, and applies damage.
